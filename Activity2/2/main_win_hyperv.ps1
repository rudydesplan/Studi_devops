...
if ($minikubeStatus -notmatch "host: Running") {
    Write-Output "Minikube is not running. Please start minikube first."
    exit 1
}

# Check if the minikube context exists
$contexts = kubectl config get-contexts -o=name
if ($contexts -notcontains "minikube") {
    Write-Output "Minikube context does not exist. Please set up minikube."
    exit 1
}
